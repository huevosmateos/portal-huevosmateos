...
def main():
    ...
    # git push (detectar nombre de la rama actual)
    print("3/3  git push ...")
    ret_b, rama = ejecutar(["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=WEB_DIR)
    rama = rama.strip() if ret_b == 0 and rama.strip() else "main"
    ret, out = ejecutar(["git", "push", "--set-upstream", "origin", rama], cwd=WEB_DIR)
    if ret != 0:
