...
def subir(sb, tabla, datos, lote=100):
    """Sube por lotes con upsert (inserta o actualiza por la clave primaria).
    ...
    for i in range(0, len(datos), lote):
        ...
        try:
            ...
            total += len(bloque)
            print(f"  [{tabla}] subidas {total}/{len(datos)}")
        except Exception as e:
            print(f"  [{tabla}] ERROR en lote {i}-{i+len(bloque)}: {e}")
            print(f"  primera fila del lote: {bloque[0]}")
            return total
    return total
