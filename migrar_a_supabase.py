...
def detectar_imagenes(cod4):
    """Busca fotos del producto en FOTOS_PATH siguiendo la convencion:
    CODIGO.jpg, CODIGO_1.jpg, CODIGO_2.jpg...
    Devuelve lista de rutas relativas: ['img/0310.jpg', 'img/0310_1.jpg']"""
    ...
    for nombre_img in candidatos:
        if os.path.isfile(os.path.join(FOTOS_PATH, nombre_img)):
...
def leer_articulos(conn, limite=None):
    ...
    for r in filas:
        ...
        salida.append({
            ...
            # publicar_web: cumple DOS condiciones
            #   1) ser vendible  (ya garantizado: si no, continue arriba)
            #   2) PublicarInternet = -1  (convencion ERP: -1 = SI publicar)
            "publicar_web":          int(num(g(r, "PublicarInternet"))) != 0,
            "vendible":              True,   # solo vendibles llegan aqui
            "imagenes":              detectar_imagenes(cod),
        })
...
def detectar_emails_duplicados(clientes):
    """Detecta si entre los clientes AUTORIZADOS (puede_acceder=True con email)
    hay alguno que repita email. Si lo hay, DESACTIVA al duplicado (le quita
    el email y el puede_acceder), y AVISA por pantalla con todos los datos
    para que el usuario los revise manualmente en el ERP.
    Como solo subimos email a los autorizados, aqui solo deberian aparecer
    como duplicados los casos REALES de conflicto."""
    por_email = {}
    for c in clientes:
        em = c.get("email")
        if em and c.get("puede_acceder"):
    ...
    if duplicados:
        print()
        print("  *** AVISO: emails duplicados entre clientes AUTORIZADOS ***")
        print("  No se activa ninguno de estos clientes para no romper la base.")
        print("  Decide en el ERP cual mantiene el email y a cuales les pones uno nuevo:")
        for em, lista in duplicados.items():
            cods = ', '.join(c["codigo"] for c in lista)
            print(f"    {em} -> clientes: {cods}")
            # Desactivamos a TODOS los del grupo: ni se loguea el primero,
            # para forzar la decision manual. Mas seguro que elegir por nosotros.
            for c in lista:
                c["email"] = None
                c["puede_acceder"] = False
        print()
        print(f"  Total: {sum(len(l) for l in duplicados.values())} clientes "
              f"afectados (en {len(duplicados)} emails repetidos)")
        print()


def leer_clientes(conn, limite=None):
    """Devuelve clientes con codigo, nombre, nombre_comercial, telefono,
    email, tarifa y descuento.
    - nombre          <- campo 'Nombre'      del ERP (razon social fiscal)
    - nombre_comercial <- campo 'RazonSocial' del ERP (nombre del negocio)
    - telefono        <- campo 'Cargo1'      del ERP (telefono principal)
    ...
    # 2) tabla Clientes (usamos * para asegurar que vienen BloqueoAlbaran
    #    y ActivarLogicNet, ademas de los campos basicos)
    salida = []
    descartados_bloqueados = 0
    ...
    for r in filas:
        ...
        # FILTRO: solo subir clientes NO bloqueados (BloqueoAlbaran = 0)
        # Convencion ERP: -1 = SI bloqueado, 0 = NO bloqueado
        bloqueado = int(num(g(r, "BloqueoAlbaran"))) != 0
        if bloqueado:
            descartados_bloqueados += 1
            continue
        ...
        # puede_acceder: ActivarLogicNet = -1 (SI puede auto-registrarse)
        puede_acceder = int(num(g(r, "ActivarLogicNet"))) != 0

        # Email: SOLO se incluye si el cliente esta autorizado a acceder.
        # Los no autorizados se suben sin email (tal y como si no existieran
        # en el portal). Asi el Auth Hook no los encuentra al intentar registrarse.
        email_cliente = None
        if puede_acceder:
            email_cliente = emails.get(cod) or None

        salida.append({
            "codigo":           cod,
            "nombre":           texto(g(r, "Nombre")) or None,
            "nombre_comercial": texto(g(r, "RazonSocial")) or None,
            "telefono":         texto(g(r, "Cargo1")) or None,
            "email":            email_cliente,
            "tarifa":           tarifa,
    ...
    if descartados_bloqueados:
        print(f"  [info] {descartados_bloqueados} clientes descartados por estar BLOQUEADOS")
...
def main():
    print("=" * 62)
    print("MIGRAR A SUPABASE - HUEVOS MATEOS")
    print("=" * 62)

    # Si se pasa argumento por linea de comandos, usarlo; si no, preguntar
    if len(sys.argv) >= 2 and sys.argv[1] in ("prueba", "completo", "contar"):
        modo = sys.argv[1]
    else:
        print()
        print("  1) prueba    - solo 5 articulos / 3 clientes / 3 ofertas")
        print("  2) completo  - todo el catalogo + limpieza de obsoletos")
        print("  3) contar    - solo mostrar cuantos hay sin escribir nada")
        print()
        opcion = input("Elige opcion (1/2/3): ").strip()
        modos = {"1": "prueba", "2": "completo", "3": "contar",
                 "prueba": "prueba", "completo": "completo", "contar": "contar"}
    ...
    con_foto = sum(1 for a in articulos if a.get("imagenes"))
    print(f"  -> {con_foto} con imagenes detectadas en {FOTOS_PATH}")
    ...
    # Limpieza de obsoletos: solo en modo 'completo'
    if modo == "completo":
