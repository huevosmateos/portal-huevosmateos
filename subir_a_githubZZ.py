...
def main():
    ...
    # git push
    print("3/3  git push ...")
    ret, out = ejecutar(["git", "push", "--set-upstream", "origin", "main"], cwd=WEB_DIR)
    if ret != 0:
        # Reintentar con master por si la rama se llama asi
        ret, out = ejecutar(["git", "push", "--set-upstream", "origin", "master"], cwd=WEB_DIR)
    if ret != 0:
